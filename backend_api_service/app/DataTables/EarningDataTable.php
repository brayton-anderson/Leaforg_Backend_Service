...
class EarningDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ->editColumn('store.name', function ($earning) {
                return getLinksColumnByRouteName([$earning->store], "stores.edit",'id','name');
            })
            ...
            ->editColumn('store_earning', function ($earning) {
                return getPriceColumn($earning,'store_earning');
            })
    ...
    protected function getColumns()
    {
        $columns = [
            [
                'data' => 'store.name',
                'title' => trans('lang.earning_store_id'),
            ...
            [
                'data' => 'store_earning',
                'title' => trans('lang.earning_store_earning'),
    ...
    public function query(Earning $model)
    {
        if (auth()->user()->hasRole('admin')) {
            return $model->newQuery()->with("store")->select('earnings.*');
        }else if((auth()->user()->hasRole('manager'))){
            return $model->newQuery()->with("store")
                ->join("user_stores", "user_stores.store_id", "=", "earnings.store_id")
                ->where('user_stores.user_id', auth()->id())->select('earnings.*');
        }
    }
