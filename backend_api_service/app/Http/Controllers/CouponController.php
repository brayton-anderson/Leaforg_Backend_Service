...
use App\Criteria\Stores\ActiveCriteria;
...
class CouponController extends Controller
{
    ...
    public function create()
    {
        ...
        $product = $this->productRepository->groupedByStores();
        ...
        return view('coupons.create')->with("customFields", isset($html) ? $html : false)->with("product", $product)->with("store", $store)->with("category", $category)->with("productsSelected", $productsSelected)->with("storesSelected", $storesSelected)->with("categoriesSelected", $categoriesSelected);
    }
        ...
        try {
            ...
            if(isset($input['stores'])){
                foreach ($input['stores'] as $storeId){
                    $discountables[] = ["discountable_type"=>"App\Models\Store","discountable_id"=>$storeId];
                }
            }
    ...
    public function edit($id)
    {
        ...
        $storesSelected = $coupon->discountables()->where("discountable_type","App\Models\Store")->pluck('discountable_id');
        ...
        return view('coupons.edit')->with('coupon', $coupon)->with("customFields", isset($html) ? $html : false)->with("product", $product)->with("store", $store)->with("category", $category)->with("productsSelected", $productsSelected)->with("storesSelected", $storesSelected)->with("categoriesSelected", $categoriesSelected);
    }
