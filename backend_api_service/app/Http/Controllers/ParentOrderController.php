...
abstract class ParentOrderController extends Controller
{
    ...
    protected function calculateDeliveryFee(): void
    {
        try {
            $carts = $this->order->user->cart;
            $this->order->delivery_fee = $carts[0]->product->store->delivery_fee;
    ...
    protected function calculateTaxFee(): void
    {
        try {
            $carts = $this->order->user->cart;
            if (empty($carts[0]->product->store->default_tax)) {
                $this->order->tax = setting('default_tax', 0);
            } else {
                $this->order->tax = $carts[0]->product->store->default_tax;
            }
    ...
    {
        Notification::send($this->order->user->cart[0]->product->store->users, new NewOrder($this->order));
    }
